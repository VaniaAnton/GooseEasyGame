...
def create_enemy():
    ...
    enemy_rect = pygame.Rect(WIDTH, random.randint(enemy.get_height(), 
...
while playing:
    ...
    for event in pygame.event.get():
        ...
        if event.type == CHANGE_IMAGE:
            if image_index < len(PLAYER_IMAGE):
                player = pygame.image.load(f'{IMAGE_PATH}/{PLAYER_IMAGE[image_index]}').convert_alpha()
                image_index += 1
            else:
                image_index = 0
    ...
    main_display.blit(FONT.render(f'Score: {score}', True, COlOR_BLACK), (WIDTH-120, 20))
